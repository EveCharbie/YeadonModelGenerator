...
def find_acromion_right(edges, ear, shoulder, height):
    ...
    acromion = np.array([ear[0] - (np.where(reversed_image_array == 255)[1][0]), ear[1] + np.where(reversed_image_array == 255)[0][0]])
    return acromion
...
def get_crotch_right_left(edges, data):
    ...
    crotch_approx_right, crotch_approx_left = np.array([data[12][0], data[12][1] + crotch_approx_crop[1]]), np.array([data[11][0], data[11][1] + crotch_approx_crop[1]])
    return crotch_approx_right, crotch_approx_left

def get_mid_thigh_right_left(data, r_crotch, l_crotch):
    mid_thigh_right, mid_thigh_left = (data[14][0:2] + r_crotch) / 2, (data[13][0:2] + l_crotch) / 2
